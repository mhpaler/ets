...
```bash
hardhat accounts --network [localhost|mumbai_stage]
```
...
```bash
hardhat addRelayer --name "My Relayer" --signer "account3" --network [localhost|mumbai_stage]
```
...
```bash
hardhat togglePauseRelayerByOwner --relayer "ETSRelayer" --signer "account1" --network [localhost|mumbai_stage]
```
...
```bash
hardhat transferRelayer --relayer "ETSRelayer" --signer "account1" --to "0x70997970C51812dc3A010C7d01b50e0d17dc79C8" --network [localhost|mumbai_stage]
```
...
```bash
hardhat createTags --relayer "ETSRelayer" --signer "account3" --tags "#USDC, #Solana" --network [localhost|mumbai_stage]
```
...
hardhat applyTags --relayer "Uniswap" --uri "https://solana.com/" --tags "#Solana,#Web" --record-type "discovery" --signer "account5" --network [localhost|mumbai_stage]

# Append #Infrastructure the tagging record we just created
hardhat applyTags --relayer "Uniswap" --uri "https://solana.com/" --tags "#Infrastructure" --record-type "discovery" --signer "account5" --network [localhost|mumbai_stage]
```
...
hardhat removeTags --relayer "Uniswap" --uri "https://solana.com/" --tags "#Solana" --record-type "discovery" --signer "account5" --network [localhost|mumbai_stage]
```
...
The only other required flag is the `--network` flag. This is simply the network you wish the task to be performed on. Current options are `localhost`, `mumbai_stage` & `mumbai`. If you don't supply the network flag, the task will almost certainly error out.
...
`hardhat auctionhouse --action settings --network [localhost|mumbai_stage]`
...
```bash
hardhat auctionhouse --action togglepause --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action setreserve --value 0.1 --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action setduration --value 60 --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action settimebuffer --value 60 --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action setmaxauctions --value 3 --network [localhost|mumbai_stage]
```
...
`hardhat auctionhouse --action showcurrent --network [localhost|mumbai_stage]`
...
```bash
hardhat auctionhouse --action status --tag [hastag] --id [auctionId] --output object --network [localhost|mumbai_stage]
```
...
```bash
hardhat auctionhouse --action nextauction --network [localhost|mumbai_stage]
```
...
```bash
hardhat auctionhouse --action auction --tag "#way" --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action bid --id "3" --signer account3 --bid 0.2 --network [localhost|mumbai_stage]
```
...
hardhat auctionhouse --action settleauction --network [localhost|mumbai_stage]
```
...
```bash
hardhat testdata --action createTag --qty 5 --signers 4 --network [localhost|mumbai_stage]
```
...
```bash
hardhat testdata --action createTaggingRecords --qty 4 --signers 5 --network [localhost|mumbai_stage]
```
...
```bash
hardhat testdata --action createAuctions --qty 3 --signers 3 --network [localhost|mumbai_stage]
```
