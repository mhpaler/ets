...
const _TAG_TYPEHASH = keccak256(toUtf8Bytes("Tag(bytes32 tagParamsHash)"));
...
function getTagStructHash(contractAddress, contractName, contractVersion, taggingRecords) {
  ...
  console.info("num of tagging records", taggingRecords.length);
  ...
  return keccak256(
    solidityPack(
      ...
      [
        ...
        "0x01",
        DOMAIN_SEPARATOR,
        keccak256(defaultAbiCoder.encode(["bytes32", "bytes32"], [_TAG_TYPEHASH, taggingRecordsHash])),
      ],
    ),
  );
}
...
function signTagRequest(contractAddress, contractName, contractVersion, taggingRecords, signerPrivateKey) {
  const messageStructHash = getTagStructHash(contractAddress, contractName, contractVersion, taggingRecords);
