...
const Bid = ({
  ...
  return (
    <div className="flex items-center justify-between py-3">
      ...
      <div className="flex-1 mr-2 truncate text-slate-700">
        {address}
      </div>
                  ...
                  <div className="relative w-full px-8 pt-8 pb-5 overflow-hidden bg-white shadow-2xl w rounded-xl">
                    ...
                    <div className="divide-y divide-slate-200">
                      {bids && bids.map((bid: Bid) => (
                        <Bid key={`all-${bid.transaction}`} address={bid.address} amount={bid.amount} transaction={bid.transaction} />
                      ))}
                    </div>
                  </div>
                </Dialog.Panel>
        ...
        <div>
          <div className="overflow-hidden shadow-xl rounded-xl shadow-slate-300">
            <svg className="w-full h-auto text-white bg-gradient-to-tr from-teal-500 via-purple-500 to-pink-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000">
              ...
              <text fill="currentColor" stroke="none" strokeMiterlimit="10" fontFamily="HelveticaNeue-Bold,Helvetica Neue" fontSize="50" fontWeight="700" transform="translate(100 150)">#hello</text>
            </svg>
          </div>
        </div>
        <div>
            ...
            <div className="col-span-2">
              <h2 className="text-3xl font-bold text-slate-700">
                <Link href={`/tags/hello`}>
                  <a className="text-pink-500 hover:text-pink-600">
                    #hello
                  </a>
                </Link>
              </h2>
          ...
          <div className="divide-y divide-slate-200">
            {bids && bids.slice(0, amountOfBidsToShow).map((bid: Bid) => (
              <Bid key={bid.transaction} address={bid.address} amount={bid.amount} transaction={bid.transaction} />
            ))}
          </div>
