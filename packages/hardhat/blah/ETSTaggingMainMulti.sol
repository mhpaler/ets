...
contract ERC721HashtagRegistry is Initializable, ContextUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    struct Target {
      string targetType;
      string targetURI; // a unique identifier for this (decryptable)
      uint created; // creation date
      uint lastEnsured; // if null, target has never been ensured.
      string ipsHash; // if not null, means target is Ensured.
      string ceramicHash; 
    }

    // targetType => ETSTargetType contract address mapping
    // ETSTargetType contracts implement IETSTargetType and provide a way to
    // isolate functions for interacting with target types from ETSTaggingMain
    mapping(string => address) public targetType;

    // _targetURI => Target mapping.
    ...
    function tag(
        string calldata _tag, // tag string eg. "#love"
        string calldata _type,
        string calldata _targetURI, // simple solution here is a delimited string to represent the URI.
        ...
        // Here we check that targetType address has been registered.
        // We store type => address in targetType.
        address targetTypeAddress = targetType[_type];
        require(targetTypeAddress !== 0, "Invalid target type");


        // Given the targetType address we can instantiate an instance of IETSTargetType
        // can call functions on that contract.
        // I may be mucking this up, but looks like we may be able to do this?
        // see https://stackoverflow.com/questions/43106483/calling-external-contract-in-solidity-dynamically
        IETSTargetType t = IETSTargetType(targetTypeAddress);
        // here we could run some simple string validation on the targetURI
        require(t.validateTargetURI(_targetURI), "Target URI not structued correctly.");

        // for the next steps, these can be broken into sub functions,
        // but I'm putting there here for easier reading.

        // Convert the URI string to 32bytes. This assumes we make the key of the target map a bytes32 string.
        // Apparently, 0.8.7 supports this https://ethereum.stackexchange.com/a/112906
        // 
        // Or, if uint as the key makes more sense, we can convert _targetURI into a uint
        // @see https://ethereum.stackexchange.com/questions/62371/convert-a-string-to-a-uint256-with-error-handling
        string text = _type + _targetURI; 
        bytes32 targetId = bytes32(bytes(text));

        // Check that the target exists, if not, add a new one.
        Target targetMap = getTarget(targetId);
        if (targetMap[targetId] == 0) {
          // Create a new, unensured target record.
          targetMap[_targetId] = Target({
          ...
          emit TargetCreated(_targetId, targetType, targetURI, created, lastEnsured, ipfsHash, ceramicHash);
        }

        // Now let's handle the ETStag.
        uint256 etsTagId = ets.getEtsTagId(_tag);
        if (etsTagId == 0) {
            etsTagId = ets.mint(_tag, _publisher, _tagger);
        }

        _tag(etsTagId, _targetId, _publisher, _tagger);
    }



    function ensureTarget(_targetType, _targetId) public {
      _ensureTarget(_targetType, _targetId);
    }

    function _ensureTarget(_targetType, _targetId) private {
      Target target = getTarget(_targetType, _targetId);
      if (target[lastEnsured] == 0) {

        // chainlink magic here...

        target[_targetId] = Target({
          targetType: _targetType,
          targetURI: _targetId,
          created: now(),
          lastEnsured: _lastEnsured, // if null, target has never been ensured.
          ipfsHash: _ipfsHash,
          ceramicHash: _ceramicHash
        });

        emit EnsureTarget(targetType, targetURI, created, lastEnsured, ipfsHash, ceramicHash);
      }
    } 

    function getTarget(string calldata targetURI) public view returns (Target target) {
