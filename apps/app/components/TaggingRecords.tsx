...
const TaggingRecords: NextPage<Props> = ({ filter, pageSize, orderBy, title }) => {
  ...
  const showPrevNext = () => {
    return (nextTaggingRecords && nextTaggingRecords.length > 0) || (skip && skip !== 0) ? true : false;
  };

  const columns = useMemo(() => [t("id"), t("created"), t("relayer"), t("record-type"), t("target"), t("tags")], [t]);

  return (
    <Table loading={!taggingRecords} rows={pageSizeSet} className="bg-inherit">
        ...
        {taggingRecords &&
          taggingRecords.map((taggingRecord: any) => (
            <Table.Tr key={taggingRecord.id}>
              <Table.Cell url={"/tagging-records/" + taggingRecord.id} value={taggingRecord.id} truncate copyAndPaste />
              <Table.CellWithChildren>
                <TimeAgo date={taggingRecord.timestamp * 1000} />
              </Table.CellWithChildren>

              <Table.CellWithChildren>
                <Link href={`/relayers/${taggingRecord && taggingRecord.relayer.id}`}>
        ...
        <Table.Footer>
          <tr>
            <td className="flex justify-between">
              ...
              <Button disabled={nextTaggingRecords && nextTaggingRecords.length === 0} onClick={() => nextPage()}>
                {t("next")}
