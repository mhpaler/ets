...
```solidity
function getOrCreateTagId(string _tag, address payable _publisher, address payable _creator) external payable returns (uint256 tokenId)
```
...
```solidity
function createTag(string _tag, address payable _publisher, address payable _creator) external payable returns (uint256 tokenId)
```
