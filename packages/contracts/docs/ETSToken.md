...
```solidity
function initialize(address _etsAccessControls, uint256 _tagMinStringLength, uint256 _tagMaxStringLength, uint256 _ownershipTermLength) public
```
...
```solidity
function getOrCreateTag(string _tag, address payable _creator) public payable returns (struct IETSToken.Tag tag)
```
...
```solidity
function tagExists(string _tag) public view returns (bool)
```
...
```solidity
function getTag(string _tag) public view returns (struct IETSToken.Tag)
```
...
```solidity
function getTag(uint256 _tokenId) public view returns (struct IETSToken.Tag)
```
