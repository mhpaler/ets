...
import { IETSTargetTypeTagger, IERC165 } from "./interfaces/IETSTargetTypeTagger.sol";
import { IETSTagTarget } from "../interfaces/IETSTagTarget.sol";
...
/// @dev UUPS upgradable.
contract EVMNFT is
    IETSTargetTypeTagger,
    TargetTypeSignatureModule,
    OwnableUpgradeable,
    UUPSUpgradeable,
    PausableUpgradeable
{
    ...
    function tag(
        ...
        address tagger = recoverAddress(taggingRecordsHash, _taggerSignature.v, _taggerSignature.r, _taggerSignature.s);

        address payable publisher = payable(
            recoverAddress(taggingRecordsHash, _publisherSignature.v, _publisherSignature.r, _publisherSignature.s)
        );
        ...
        for (uint256 i; i < _taggingRecords.length; ++i) {
            ...
            _tag(_taggingRecords[i], publisher, tagger, currentTaggingFee);
        }
    ...
    function isTargetTypePaused() external view override returns (bool) {
        return paused();
    }
