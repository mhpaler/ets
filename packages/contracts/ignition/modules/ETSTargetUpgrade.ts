...
/**
 * Ignition module for upgrading ETSTarget to ETSTargetUpgrade
 *
 * This demonstrates how to handle UUPS upgrades with Ignition
 */
const ETSTargetUpgradeModule = buildModule("ETSTargetUpgrade", (m) => {
  // Get the proxy address from parameters
  const proxyAddress = m.getParameter("proxyAddress");

  // Deploy the new implementation
  const newImplementation = m.contract("ETSTargetUpgrade", []);

  // Get reference to the existing proxy
  const proxy = m.contractAt("ETSTarget", proxyAddress);
...
export default ETSTargetUpgradeModule;
