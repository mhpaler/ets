...
describe("ETS Auction House Tests", function () {
  ...
  beforeEach("Setup test", async function () {
    ...
    etsOwnedTag = "#Love";
    await contracts.ETSToken.connect(accounts.RandomTwo).createTag(etsOwnedTag, accounts.ETSPublisher.address);
    ...
    await contracts.ETSToken.connect(accounts.RandomTwo).createTag(userOwnedTag, accounts.ETSPublisher.address);
  ...
  describe("Only administrators", async function () {
    ...
    it("can pause & unpause the auction", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).pause())
        .to.be.revertedWith(
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.ETSAdmin).pause())
          .to.emit(
            contracts.ETSAuctionHouse, "Paused"
          )
          .withArgs(
            accounts.ETSAdmin.address
          );
          
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).unpause())
        .to.be.revertedWith(
          "Caller must be administrator",
        );
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.ETSAdmin).unpause())
        .to.emit(
          contracts.ETSAuctionHouse, "Unpaused"
        )
        .withArgs(
          accounts.ETSAdmin.address
        );
    ...
    it("can set the reserve price", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).setReservePrice(1000))
        .to.be.revertedWith(
          "Caller must be administrator",
        );
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.ETSAdmin).setReservePrice(1000))
        .to.emit(
          contracts.ETSAuctionHouse, "AuctionReservePriceSet"
        )
        .withArgs(
          1000
        );
    ...
    it("can set the auction proceed percentages", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).setProceedPercentages(25, 25))
      .to.be.revertedWith(
        "Caller must be administrator",
      );
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.ETSAdmin).setProceedPercentages(25, 25))
        .to.emit(
          contracts.ETSAuctionHouse, "AuctionProceedPercentagesSet"
        )
        .withArgs(
          25, 25, 50
        );
    ...
    it("can set the auction time buffer", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).setTimeBuffer(10))
      .to.be.revertedWith(
        "Caller must be administrator",
      );
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.ETSAdmin).setTimeBuffer(10))
        .to.emit(
          contracts.ETSAuctionHouse, "AuctionTimeBufferSet"
        )
        .withArgs(
          10
        );
  ...
  describe("Bidding to create a new auction", async function () {
    ...
    it("should revert when auction is paused", async function () {
      ...
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).createBid(userOwnedTagId, { value: initSettings.RESERVE_PRICE }))
        .to.be.revertedWith(
          "Pausable: paused",
        );
    });
    ...
    it("should revert when token doesn't exist", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).createBid(constants.Two, { value: initSettings.RESERVE_PRICE }))
        .to.be.revertedWith(
    ...
    it('should revert if a user creates a bid with an amount below the reserve price', async () => {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE - 1 }))
        .to.be.revertedWith(
    ...
    it ("should create new auction if token exists and active auction not found", async function () {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE }))
        .to.emit(
          contracts.ETSAuctionHouse, "AuctionCreated"
        )
    ...
    it('should emit an `AuctionBid` event on a successful starting bid', async () => {
      await expect(contracts.ETSAuctionHouse.connect(accounts.RandomTwo).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE }))
        .to.emit(
          contracts.ETSAuctionHouse, "AuctionBid"
        )
        .withArgs(
          etsOwnedTagId,
          accounts.RandomTwo.address,
   ...
   describe("Bidding on active auction", async function () {
     beforeEach(async function () {
       ...
       await contracts.ETSAuctionHouse.connect(accounts.RandomOne).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE });
 
     });
     it ("should revert if bid doesn't meet min bid increment", async function () {
       const auction = await contracts.ETSAuctionHouse.getAuction(etsOwnedTagId);
       const low_bid_increment = Number(auction.amount) + (Number(auction.amount) * ((initSettings.MIN_INCREMENT_BID_PERCENTAGE - 3)/100));
       await expect(contracts.ETSAuctionHouse.connect(accounts.RandomTwo).createBid(etsOwnedTagId, { value: low_bid_increment }))
         .to.be.revertedWith(
     ...
     it ("should emit an `AuctionBid` event on a successful second bid", async function () {
       ...
       await expect(contracts.ETSAuctionHouse.connect(accounts.RandomTwo).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE * 2 }))
         .to.emit(
           contracts.ETSAuctionHouse, "AuctionBid"
         )
         .withArgs(
     ...
     it ("should return funds to previous bidder on good bid", async function () {
       ...
       // RandomTwo successfully increments the bid.
       await contracts.ETSAuctionHouse.connect(accounts.RandomTwo).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE * 2 });
     ...
     it('should emit an `AuctionExtended` event if the auction end time is within the time buffer', async () => {
       ...
       await ethers.provider.send('evm_setNextBlockTimestamp', [auction.endTime.sub(initSettings.TIME_BUFFER/2).toNumber()]);
       
       await expect(contracts.ETSAuctionHouse.connect(accounts.RandomTwo).createBid(etsOwnedTagId, { value: initSettings.RESERVE_PRICE * 2 }))
         .to.emit(
           contracts.ETSAuctionHouse, "AuctionExtended"
         )
         .withArgs(
           etsOwnedTagId,
           auction.endTime.add(initSettings.TIME_BUFFER/2)
         );
     });
   ...
   describe("Settling an ended auction", async function () {
     ...
     it ("should revert for non-existent auction", async function () {
       ...
       await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).settleAuction(constants.Two))
       .to.be.revertedWith(
     ...
     it ("should send CTAG token to winning bidder", async function () {
       ...
       await expect(contracts.ETSAuctionHouse.connect(accounts.RandomOne).settleAuction(etsOwnedTagId))
         .to.emit(
           contracts.ETSAuctionHouse, "AuctionSettled"
         );
     ...
     it ("should send auction proceeds to actors", async function () {
       ...
       const ctag = await contracts.ETSToken.getTag(etsOwnedTagId);
