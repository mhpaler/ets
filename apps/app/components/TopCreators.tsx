    ...
    <div className="w-full mx-auto">
      <Link href="/">
        <a className="flex justify-between border border-b-0 border-slate-900">
          <div>
            <h2 className="px-6 py-3 text-xs tracking-wider text-left text-black uppercase">Top creators</h2>
          </div>
        ...
        {data && data.creators.map(creator => (
          <div className="grid grid-flow-col grid-cols-2 px-6 py-4 space-x-4" key={creator.id}>
            <div>
              ...
              <div className="text-sm leading-6 text-slate-500">{creator.mintCount} tags</div>
            </div>
            <div className="col-span-2">
              <div className="overflow-hidden text-slate-700 text-ellipsis whitespace-nowrap">{toDp(toEth(creator.tagFees))} MATIC earned</div>
              <div className="text-sm leading-6 text-slate-500">{creator.tagCount} tagged</div>
            </div>
          </div>
        ))}
      </div>
