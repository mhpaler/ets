...
describe("ETS Channel Tests", async () => {
  ...
  describe("Creating tags", async () => {
    it("should create tags successfully", async () => {
      ...
      const _tagAddressesBefore = await contracts.ETSChannel.write.getOrCreateTagIds([[tagstring1]], {
  ...
  describe("Applying tags", async () => {
    it("should revert when Channel contract is paused", async () => {
      const isActiveInitially = await contracts.ETSAccessControls.read.isChannelAndNotPaused([
        contracts.ETSChannel.address,
      ]);
      assert.equal(isActiveInitially, true, "ETSChannel should start active");
    ...
    it("should create new tagging record successfully", async () => {
      // First ensure ETSChannel is active
      const isChannelActive = await contracts.ETSAccessControls.read.isChannelAndNotPaused([
        contracts.ETSChannel.address,
      ]);
      ...
      if (!isChannelActive) {
        ...
        const _isActiveAfter = await contracts.ETSAccessControls.read.isChannelAndNotPaused([
          contracts.ETSChannel.address,
        ]);
      }
    ...
    it("should update existing tagging record when appending tags", async () => {
      ...
      await contracts.ETSChannel.write.applyTags([[appendRecord]], {
    ...
    it("should not update tagging record when same tags are applied", async () => {
      ...
      for (let i = 0; i < taggingRecords.length; i++) {
        const result = await contracts.ETSChannel.read.computeTaggingFee([taggingRecords[i], 0]);
        const [fee] = result;
        _calcTaggingFee += fee;
      }
  ...
  describe("Removing tags", async () => {
    it("should revert when Channel contract is paused", async () => {
      ...
      try {
        await contracts.ETSChannel.write.removeTags([taggingRecords], {
          account: accounts.User2.account,
        });
        assert.fail("Should have reverted");
      } catch (error: any) {
    ...
    it('should emit "TaggingRecordUpdated" and reduce tag count', async () => {
      ...
      const _tx = await contracts.ETSChannel.write.removeTags([[removeTags]], { account: accounts.User2.account });
      // TODO: Event testing needs to be implemented with viem
    ...
    it("can work with multiple tags at once", async () => {
      ...
      await contracts.ETSChannel.write.removeTags([[tagParams]], { account: accounts.User2.account });
      const taggingRecord = await contracts.ETS.read.getTaggingRecordFromId([taggingRecordId]);
  ...
  describe("Replacing tags", async () => {
    it("should revert when Channel contract is paused", async () => {
      ...
      try {
        await contracts.ETSChannel.write.replaceTags([[replaceTags]], {
          value: taggingFee * BigInt(2),
          account: accounts.User2.account,
        });
        assert.fail("Should have reverted");
    ...
    it("should revert if tagging record doesn't exist", async () => {
      ...
      try {
        await contracts.ETSChannel.write.replaceTags([taggingRecords], {
          value: taggingFee * BigInt(2),
          account: accounts.User2.account,
        });
        assert.fail("Should have reverted");
    ...
    it('should emit "TaggingRecordUpdated" and change tag count', async () => {
      ...
      const _tx = await contracts.ETSChannel.write.replaceTags([[replaceTags]], {
    ...
    it('should not emit "TaggingRecordUpdated" or reduce tag count if nothing changes', async () => {
      ...
      let result: { 0: bigint };
      for (let i = 0; i < taggingRecords.length; i++) {
        result = await contracts.ETSChannel.read.computeTaggingFee([
          taggingRecords[i],
          1, // action to perform
        ]);
        const { 0: fee } = result;
      ...
      const _tx = await contracts.ETSChannel.write.replaceTags([taggingRecords], {
    ...
    it("should only charge for new tags", async () => {
      ...
      const _tx1 = await contracts.ETSChannel.write.applyTags([taggingRecords], {
      ...
      const _tx2 = await contracts.ETSChannel.write.replaceTags([replaceRecords], {
  ...
  describe("Tagging fees", async () => {
    describe("for new tagging records", async () => {
      it("should fail when providing an invalid tagging action", async () => {
        ...
        try {
          await contracts.ETSChannel.read.computeTaggingFee([
            rawInput,
      ...
      it("are computed correctly with raw tagging record parts", async () => {
        ...
        const result = await contracts.ETSChannel.read.computeTaggingFee([rawInput, 0]);
    ...
    describe("for existing tagging records", async () => {
      beforeEach("Setup test", async () => {
        // Create a tagging record, ETSPlatform is channel, accounts.User2 is tagger.
        await contracts.ETSChannel.write.applyTags([taggingRecords], {
          value: taggingFee * BigInt(2),
          account: accounts.User2.account,
        });
      ...
      it("are computed correctly when replacing with only new tags using raw inputs", async () => {
        ...
        const result = await contracts.ETSChannel.read.computeTaggingFee([rawInput, 1]);
  ...
  describe("Cross-channel operations", () => {
    beforeEach(async () => {
      // Create initial tagging record using first channel
      await contracts.ETSChannel.write.applyTags([taggingRecords], {
        value: taggingFee * BigInt(2),
        account: accounts.User2.account,
      });
    ...
    it("attributes new tagging records to calling channel regardless of channel parameter", async () => {
      const newRecord = {
      ...
      // Create record through secondChannel but try to attribute it to firstChannel
      await contracts.secondChannel.write.applyTagsViaChannel([[newRecord], contracts.ETSChannel.address], {
        value: taggingFee,
        account: accounts.User2.account,
      });

      // Verify the record was created with secondChannel as the channel
      const taggingRecordId = await contracts.ETS.read.computeTaggingRecordIdFromRawInput([
        newRecord,
        contracts.secondChannel.address,
        accounts.User2.account.address,
      ]);

      const record = await contracts.ETS.read.getTaggingRecordFromId([taggingRecordId]);
      assert.equal(record[3], contracts.secondChannel.address);
    });

    it("allows tagger to add tags to their record through second channel", async () => {
      const appendRecord = {
        targetURI: targetURI,
        tagStrings: ["#newtag1"],
        recordType: "bookmark",
      ...
      const _tx = await contracts.secondChannel.write.applyTagsViaChannel(
        [[appendRecord], contracts.ETSChannel.address],
        {
          value: taggingFee,
          account: accounts.User2.account,
        },
    ...
    it("allows tagger to replace tags through second channel", async () => {
      const replaceRecord = {
        targetURI: targetURI,
      ...
      const _tx = await contracts.secondChannel.write.replaceTagsViaChannel(
        [[replaceRecord], contracts.ETSChannel.address],
        {
          value: taggingFee,
          account: accounts.User2.account,
        },
    ...
    it("allows tagger to remove tags through second channel", async () => {
      const removeRecord = {
        targetURI: targetURI,
        tagStrings: ["#love"],
        recordType: "bookmark",
      ...
      const _tx = await contracts.secondChannel.write.removeTagsViaChannel(
        [[removeRecord], contracts.ETSChannel.address],
        {
          account: accounts.User2.account,
        },
      );
